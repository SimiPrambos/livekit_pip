...
- **iOS** — drives a native `AVSampleBufferDisplayLayer` with a live WebRTC frame pipeline. The active speaker fills the PiP window automatically, with an optional self-view inset composited in. No extra entitlements needed on iOS 18+ with a `voip` background mode.
...
- Screen-share suppression (don't enter PiP when the local user is screen sharing)
